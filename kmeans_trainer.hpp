...
class KMeansTrainer
{
  public:
    ...
    void initialize(bob::learn::em::KMeansMachine& kMeansMachine,
      const xt::xexpression<E>& sampler);
    ...
    void eStep(bob::learn::em::KMeansMachine& kmeans,
      const xt::xexpression<E>& data);

    /**
     * @brief Updates the mean based on the statistics from the E-step.
     */
    template <class E>
    void mStep(bob::learn::em::KMeansMachine& kmeans, const xt::xexpression<E>& /*unused*/);
     ...
     */
    void setRng(const std::shared_ptr<std::mt19937> rng)
    { m_rng = rng; }

    /**
     * @brief Gets the Random Number Generator
     */
    const std::shared_ptr<std::mt19937> getRng() const
    { return m_rng; }
     ...
     */
    const xt::xtensor<double, 1>& getZeroethOrderStats() const { return m_zeroethOrderStats; }
    const xt::xtensor<double, 2>& getFirstOrderStats() const { return m_firstOrderStats; }
     ...
     */
    void setZeroethOrderStats(const xt::xtensor<double, 1>& zeroethOrderStats);
    void setFirstOrderStats(const xt::xtensor<double, 2>& firstOrderStats);
  ...
  private:
     ...
     */
    xt::xtensor<double, 1> m_zeroethOrderStats;
     ...
     * equation 9.4, Bishop, "Pattern recognition and machine learning", 2006
     */
    xt::xtensor<double, 2> m_firstOrderStats;
};
